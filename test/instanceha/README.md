...
| `test_heartbeat_detection.py` | HBV2 protocol, HMAC-SHA256, timestamp freshness, grace period, reachable host filtering |
...
| `test_leader_election.py` | Lease acquire/renew/expire, heartbeat count inheritance, pod label patching |
...
Exception classes must be real subclasses (not MagicMock) to work with `except` clauses:
...
Mock `time.sleep()` and timing constants for fast execution:
