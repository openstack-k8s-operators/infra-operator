...
if [ $? -eq 0 ]; then
    ...
    echo "sentinel monitor redis ${master} {{ .redisPort }} ${SENTINEL_QUORUM}" >> $SENTINEL_CONFIG
...
if is_bootstrap_pod $POD_NAME; then
    ...
    echo "sentinel monitor redis ${POD_FQDN} {{ .redisPort }} ${SENTINEL_QUORUM}" >> $SENTINEL_CONFIG
    exec redis-sentinel $SENTINEL_CONFIG
fi
