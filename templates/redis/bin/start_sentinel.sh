...
if [ $? -eq 0 ]; then
    ...
    echo "sentinel monitor redis ${master} 6379 ${SENTINEL_QUORUM}" >> /var/lib/redis/sentinel.conf
...
if is_bootstrap_pod $POD_NAME; then
    ...
    echo "sentinel monitor redis ${POD_FQDN} 6379 ${SENTINEL_QUORUM}" >> /var/lib/redis/sentinel.conf
    exec redis-sentinel /var/lib/redis/sentinel.conf
fi
