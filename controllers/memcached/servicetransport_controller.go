...
func (r *ServiceTransportReconciler) reconcileNormal(ctx context.Context, instance *memcachedv1.ServiceTransport, helper *helper.Helper) (ctrl.Result, error) {
	...
	tlsEnabled := false
	if memcached.Spec.TLS.SecretName != nil {
		tlsEnabled = *memcached.Spec.TLS.SecretName != ""
	}
	Log.Info(fmt.Sprintf("memcached %s TLS enabled: %t", memcached.Name, tlsEnabled))

	// Create a new secret with the transport URL for this CR
	secret := r.createServiceTransportSecret(instance, memcached.Status.ServerList, false)
...
func (r *ServiceTransportReconciler) createServiceTransportSecret(
	...
	// TLS only works with pymemcache
	config := `[cache]
enabled=true
tls_enabled=%t
...
`
	...
	return &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "memcached-service-transport-" + instance.Name,
			Namespace: instance.Namespace,
		},
		Data: map[string][]byte{
			"config_cache":  []byte(fmt.Sprintf(config, tlsEnabled, hostList)),
			"transport_url": []byte(hostList),
		},
	}
}
