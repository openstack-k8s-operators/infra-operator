...
func (r *BGPConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	pPod := predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			// * the oldConfig and newConfig is the same
			oldConfigured := true
			var oldConfig string
			var ok bool
			if oldConfig, ok = e.ObjectOld.GetAnnotations()[k8s_networkv1.NetworkAttachmentAnnot]; !ok || len(oldConfig) == 0 {
				oldConfigured = false
			}
			newConfigured := true
			if newConfig, ok := e.ObjectNew.GetAnnotations()[k8s_networkv1.NetworkAttachmentAnnot]; !ok || oldConfig == newConfig {
				newConfigured = false
				return false
			}
...
func (r *BGPConfigurationReconciler) reconcileNormal(ctx context.Context, instance *networkv1.BGPConfiguration, helper *helper.Helper) (ctrl.Result, error) {
	...
	for _, nodeName := range getNodesRunningPods(podNetworkDetailList) {
		...
		for _, pod := range pods.Items {
			...
			if netAttachString, ok := pod.Annotations[k8s_networkv1.NetworkAttachmentAnnot]; ok && netAttachString != "[]" {
				...
				for idx, netStat := range netsStatusCopy {
					...
					ipam, err := getIPAMFromNAD(*netAtt)
					if err != nil {
						return detailList, err
					}
...
func (r *BGPConfigurationReconciler) createOrPatchFRRConfiguration(
	...
	podPrefixes := getFRRPodPrefixes(podDtl.networkStatus)
	...
	for _, r := range nodeFRRCfg.Spec.BGP.Routers {
		routers = append(routers, frrk8sv1.Router{
			ASN:       r.ASN,
			Neighbors: getFRRNeighbors(r.Neighbors, podPrefixes),
...
func getIPAMFromNAD(netAtt k8s_networkv1.NetworkAttachmentDefinition) (string, error) {
	var data interface{}
