...
func TestDeploymentSecurityContext(t *testing.T) {
	...
	// instanceha runs under restricted-v2 with a random, admission-assigned
	// UID/GID, so the manifest must not pin RunAsUser/RunAsGroup.
	g.Expect(container.SecurityContext.RunAsUser).To(BeNil())
	g.Expect(container.SecurityContext.RunAsGroup).To(BeNil())
