...
func remediationFixture(t *testing.T, nodes ...client.Object) (*PodRemediatorReconciler, *remediationv1.PodRemediator) {
	...
	pvc := &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Name: "claim", Namespace: "test", Annotations: map[string]string{remediationv1.PVCStuckOnNodeAnnotation: "worker-0", remediationv1.SafeToDeleteAnnotation: "true"}}, Spec: corev1.PersistentVolumeClaimSpec{VolumeName: "pv"}}
	...
	return &PodRemediatorReconciler{Client: c, DynamicClient: dyn}, &remediationv1.PodRemediator{ObjectMeta: metav1.ObjectMeta{Name: "pr", Namespace: "test"}}
}

func remediationNode(ready corev1.ConditionStatus) *corev1.Node {
	return &corev1.Node{ObjectMeta: metav1.ObjectMeta{Name: "worker-0"}, Status: corev1.NodeStatus{Conditions: []corev1.NodeCondition{{Type: corev1.NodeReady, Status: ready}}}}
}
...
func TestDeletedNodeConsentCompletes(t *testing.T) {
	...
	if !apierrors.IsNotFound(err) {
		t.Fatalf("consented PVC on deleted node remains; get error=%v", err)
	}
}
...
func TestEmptyNamespaceDoesNotSelectOtherNamespaces(t *testing.T) {
	ctx := context.Background()
	...
	pr.Spec.Namespaces = []string{""}
	_, err := r.reconcileNormal(ctx, pr)
	if err == nil {
		t.Fatal("empty namespace was not rejected")
	}
	if _, err := r.reconcileDelete(ctx, pr, nil); err == nil {
		t.Fatal("finalizer cleanup accepted an empty namespace")
	}
	if err := r.Get(ctx, client.ObjectKey{Namespace: "unrelated", Name: "claim"}, &corev1.PersistentVolumeClaim{}); err != nil {
		t.Fatalf("empty namespace entry remediated unrelated namespace: %v", err)
	}
}
...
func TestWatchedNamespaces(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "explicit", namespaces: []string{"openstack", "openstack-cell1"}, valid: true},
		{name: "empty entry", namespaces: []string{""}},
		{name: "mixed", namespaces: []string{"openstack", ""}},
		{name: "uppercase", namespaces: []string{"Openstack"}},
		{name: "whitespace", namespaces: []string{" "}},
		{name: "too long", namespaces: []string{strings.Repeat("a", 64)}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, err := watchedNamespaces(&remediationv1.PodRemediator{ObjectMeta: metav1.ObjectMeta{Namespace: "test"}, Spec: remediationv1.PodRemediatorSpec{Namespaces: tc.namespaces}})
			if (err == nil) != tc.valid {
