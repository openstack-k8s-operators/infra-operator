...
// Package main is the entry point for the infra-operator.
package main
...
import (
	...
	instancehacontroller "github.com/openstack-k8s-operators/infra-operator/internal/controller/instanceha"
	memcachedcontroller "github.com/openstack-k8s-operators/infra-operator/internal/controller/memcached"
	...
	topologycontroller "github.com/openstack-k8s-operators/infra-operator/internal/controller/topology"
	...
	instancehav1 "github.com/openstack-k8s-operators/infra-operator/api/instanceha/v1beta1"
	memcachedv1 "github.com/openstack-k8s-operators/infra-operator/api/memcached/v1beta1"
	networkv1 "github.com/openstack-k8s-operators/infra-operator/api/network/v1beta1"
	rabbitmqv1beta1 "github.com/openstack-k8s-operators/infra-operator/api/rabbitmq/v1beta1"
	redisv1 "github.com/openstack-k8s-operators/infra-operator/api/redis/v1beta1"
	topologyv1beta1 "github.com/openstack-k8s-operators/infra-operator/api/topology/v1beta1"
...
func main() {
	...
	if err := (&rabbitmqcontroller.TransportURLReconciler{
	...
	if err := (&memcachedcontroller.Reconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&instancehacontroller.Reconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&networkcontroller.DNSMasqReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	...
	if err := (&networkcontroller.DNSDataReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&networkcontroller.NetConfigReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&networkcontroller.ReservationReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&networkcontroller.IPSetReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(context.Background(), mgr); err != nil {
	...
	if err := (&topologycontroller.Reconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&networkcontroller.BGPConfigurationReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(context.Background(), mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookmemcachedv1beta1.SetupMemcachedWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Memcached")
			os.Exit(1)
		}
		if err := webhookinstancehav1beta1.SetupInstanceHaWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "InstanceHa")
			os.Exit(1)
		}
		if err := webhookredisv1beta1.SetupRedisWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Redis")
			os.Exit(1)
		}
		if err := webhooknetworkv1beta1.SetupDNSMasqWebhookWithManager(mgr); err != nil {
		...
		if err := webhooknetworkv1beta1.SetupReservationWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Reservation")
			os.Exit(1)
		}
		if err := webhooknetworkv1beta1.SetupIPSetWebhookWithManager(mgr); err != nil {
